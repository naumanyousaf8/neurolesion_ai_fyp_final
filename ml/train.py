...
def evaluate(model, loader, device) -> float:
    """Average :func:`ml.losses.dice_score` over an entire data-loader.
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            total += dice_score(model(x), y) * x.size(0)
            n += x.size(0)
    model.train()
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        epoch_loss, epoch_dice, n = 0.0, 0.0, 0; e_t0 = time.time()
        ...
        val_dice = evaluate(model, val_loader, device); dt = time.time() - e_t0
        print(f"[epoch {epoch:02d}/{args.epochs}] loss={train_loss:.4f} "
              f"train_dice={train_dice:.4f} val_dice={val_dice:.4f} time={dt/60:.1f}min")
