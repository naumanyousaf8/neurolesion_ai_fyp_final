...
def voxel_metrics(pred: np.ndarray, gt: np.ndarray, smooth: float = 1.0):
    """Return voxel-level ``(dice, precision, recall)`` over the entire dataset.
...
def main():
    ...
    dices, all_preds, all_gts, all_imgs = [], [], [], []
    with torch.no_grad():
        for x, y in loader:
    ...
    vd, vp, vr = voxel_metrics(preds, gts); ms = float(np.mean(dices))
    ...
    with open(out_dir / "test_metrics.json", "w") as f:
        json.dump({"checkpoint": str(args.ckpt), "n_test_slices": int(len(test_ds)),
                   ...
                   "voxel_dice": vd, "voxel_precision": vp, "voxel_recall": vr}, f, indent=2)
